...
func _compute_scale() -> float:
	...
	if min_dim <= 900:
		...
		s = 1.0
	# Viewport-fit cap: avoid upscaling on small windows (e.g., 1152x648)
	# Baseline: 1280x720. If either dimension is smaller, cap to 1.0.
	var fit_cap: float = min(float(size.x) / 1280.0, float(size.y) / 720.0)
	var cap: float = max(1.0, fit_cap) # never go below 1.0
	s = min(s, cap)
